...
type FromStatusOpts struct {
	TrackerURLs []string // full tracker URLs from FetchTrackerData
}
...
// TrackerStatus is read directly from the library's struct (it already
// requests "tracker_status" in its statusKeys).
// The opts parameter is optional — if provided, it supplies full tracker
// URLs from the raw RPC call.
func FromStatus(ts *delugeclient.TorrentStatus, label string, opts ...FromStatusOpts) TorrentInfo {
	info := TorrentInfo{
		...
		TrackerStatus: ts.TrackerStatus,
	}

	if len(opts) > 0 && len(opts[0].TrackerURLs) > 0 {
		info.Trackers = opts[0].TrackerURLs
	}
